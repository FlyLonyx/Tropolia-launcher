...
const { Launch, Status } = require('tropolia_core');
...
class Home {
    ...
    async startInitialisation() {
        document.querySelector('.launch-button').addEventListener('click', async () => {
            ...
            let javaPath = (await this.database.get('1234', 'java-path'))?.value?.path;
            let playBtn = document.querySelector('.launch-button');
            let info = document.querySelector(".text-download")
            let progressBar = document.querySelector(".progress-bar")

            playBtn.style.background = `linear-gradient(90deg,#29422c 0px,#376b3c 100%)`;
            ...
            let opts = {
                url: this.config.game_url === "" || this.config.game_url === undefined ? `${urlpkg}/filess` : this.config.game_url,
                ...
                JVM_ARGS: [`-XX:+DisableAttachMechanism`, `-noverify`],
            ...
            launch.on('data', (e) => {
                ...
                playBtn.style.background = `linear-gradient(90deg,#48754D 0px,#55A45D 100%)`;
